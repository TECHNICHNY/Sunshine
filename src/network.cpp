...
namespace net {
  ...
  std::uint16_t map_port(int port) {
    ...
    // QDEE: port_offset added on top for multi-instance isolation (S1 patch)
    auto mapped_port = (std::uint16_t) ((int) config::sunshine.port + port + config::sunshine.port_offset);
